...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.tree.configure(height=8)   # ----------------------------------------- modif 

        frame_tree = ttk.Frame(self)
        frame_tree.pack(fill="x", expand=False, padx=10, pady=(0, 6))
        self.tree.pack(in_=frame_tree, side="left", fill="both", expand=True)
        vsb.pack(in_=frame_tree, side="right", fill="y")

        # ---- NEW: Modified files list at bottom ----
        # mod_frame = ttk.LabelFrame(self, text="Fichiers modifiés")
        # mod_frame.pack(fill="x", padx=10, pady=(0, 10))

        # self.mod_list = tk.Listbox(mod_frame, height=7)
        
        mod_frame = ttk.LabelFrame(self, text="Fichiers modifiés")
        mod_frame.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        ...
        # Optional: double click opens folder selection (not requested) -> omitted.
    ...
    def scan(self):
        ...
        self._clear_tree()
        self._clear_modified_list()  # reset modified list at each scan
    ...
    def run(self):
        ...
        self._clear_modified_list()  # NEW: clear at each execution
    ...
    def _worker_rename(self, items: list[RenameItem], log_path: str, dry_run: bool, collision_mode: str):
        ...
        for it in items:
            ...
            # NEW: push to UI list when it is (or would be) modified successfully
        ...
        try:
            while True:
                ...
                if msg == "progress":
                    ...
                    # Affichage compact (mais avec chemins utiles)
                    # Tu peux changer en basename seulement si tu préfères
                    line = f"[{status}] {old_path}  ->  {new_path}"
                    self.mod_list.insert(tk.END, line)
                    self.mod_list.yview_moveto(1.0)  # auto-scroll en bas
